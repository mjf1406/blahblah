...
type EncounterWithOptimistic = (Doc<"encounters"> | {
  _id: string;
  name: string;
  description: string;
  difficulty: string;
...
export function EncounterManager() {
  const serverEncounters = useQuery(api.encounters.list) || [];
  ...
  const [editingEncounter, setEditingEncounter] = useState<Doc<"encounters"> | null>(null);
  const [name, setName] = useState("");
  const [description, setDescription] = useState("");
  const [difficulty, setDifficulty] = useState("");
  const [partyLevel, setPartyLevel] = useState(1);
  const [monsters, setMonsters] = useState<{ name: string; quantity: number; cr: string }[]>([]);
  const [optimisticEncounters, setOptimisticEncounters] = useState<Array<{
    _id: string;
    name: string;
    description: string;
    difficulty: string;
  ...
  useEffect(() => {
    if (serverEncounters.length > 0 && optimisticEncounters.length > 0) {
      const filteredOptimistic = optimisticEncounters.filter(optimistic => 
        !serverEncounters.some(server => 
  ...
  const encounters = useMemo((): EncounterWithOptimistic[] => {
    const combined: EncounterWithOptimistic[] = [
      ...serverEncounters
        .filter(e => !optimisticEdits.has(e._id))
        .map(e => ({ ...e, isOptimistic: false as const })),
  ...
  const updateMonster = (index: number, field: keyof typeof monsters[0], value: string | number) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setOptimisticEncounters(prev => prev.filter(e => e._id !== optimisticEncounter._id));
    ...
    <div className="space-y-6">
                    ...
                    <div key={index} className="flex items-center gap-2 p-2 border rounded">
                      ...
                      <div className="w-20">
                        <Input
                          ...
                          onChange={(e) => updateMonster(index, 'quantity', parseInt(e.target.value) || 1)}
                        />
                      </div>
                      <div className="w-20">
                        <Input
          ...
          <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleEditSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <Label htmlFor="editPartyLevel">Party Level</Label>
                  <Input
                    id="editPartyLevel"
                    type="number"
                    min="1"
      ...
      {encounters.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 text-lg">No encounters created yet</p>
          <p className="text-gray-400">Click "Add Encounter" to get started</p>
        </div>
      ) : (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {encounters.map((encounter) => (
            <Card key={encounter._id} className={`hover:shadow-md transition-shadow ${encounter.isOptimistic ? 'opacity-70 animate-pulse' : ''}`}>
              <CardHeader className="relative">
                  ...
                  {!encounter.isOptimistic && (
                    <div className="absolute top-4 right-4 flex gap-1">
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => handleEdit(encounter as Doc<"encounters">)}
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          ...
                          <AlertDialogFooter>
                            ...
                            <AlertDialogAction
                              onClick={() => handleDelete(encounter._id)}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="outline">Level {encounter.partyLevel}</Badge>
                </div>
              </CardHeader>
              <CardContent>
