...
type MapWithOptimistic = (Doc<"maps"> | {
  _id: string;
  name: string;
  description: string;
  type: string;
...
export function MapManager() {
  const serverMaps = useQuery(api.maps.list) || [];
  ...
  const [optimisticMaps, setOptimisticMaps] = useState<Array<{
    _id: string;
    name: string;
    description: string;
    type: string;
  ...
  useEffect(() => {
    if (serverMaps.length > 0 && optimisticMaps.length > 0) {
      const filteredOptimistic = optimisticMaps.filter(optimistic => 
        !serverMaps.some(server => 
  ...
  }, [serverMaps.length, JSON.stringify(optimisticMaps)]);
  ...
  const maps = useMemo((): MapWithOptimistic[] => {
    const combined: MapWithOptimistic[] = [
      ...serverMaps
        .filter(m => !optimisticEdits.has(m._id))
        .map(m => ({ ...m, isOptimistic: false as const })),
      ...optimisticMaps
    ];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const newMap = {
      ...
      dimensions: dimensions.width > 0 && dimensions.height > 0 ? dimensions : undefined,
    ...
    try {
      ...
      setOptimisticMaps(prev => prev.filter(m => m._id !== optimisticMap._id));
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      case "Battle Map": return "bg-red-100 text-red-800";
      case "World Map": return "bg-blue-100 text-blue-800";
      case "Dungeon Map": return "bg-purple-100 text-purple-800";
      case "City Map": return "bg-green-100 text-green-800";
      case "Regional Map": return "bg-yellow-100 text-yellow-800";
    ...
    <div className="space-y-6">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Select value={type} onValueChange={setType}>
                    ...
                    <SelectContent>
                      <SelectItem value="Battle Map">Battle Map</SelectItem>
                      <SelectItem value="World Map">World Map</SelectItem>
                      <SelectItem value="Dungeon Map">Dungeon Map</SelectItem>
                      <SelectItem value="City Map">City Map</SelectItem>
                      <SelectItem value="Regional Map">Regional Map</SelectItem>
                ...
                <div>
                  <Label htmlFor="gridSize">Grid Size (Optional)</Label>
                  <Select value={gridSize} onValueChange={setGridSize}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="10ft">10 feet</SelectItem>
              ...
              <div>
                <Label htmlFor="world">Associated World (Optional)</Label>
          ...
          <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleEditSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <Label htmlFor="editGridSize">Grid Size (Optional)</Label>
                  <Select value={gridSize} onValueChange={setGridSize}>
                    <SelectTrigger>
                      <SelectValue placeholder="Select grid size" />
                    </SelectTrigger>
              ...
              <div>
                <Label htmlFor="editWorld">Associated World (Optional)</Label>
                <Select value={worldId} onValueChange={setWorldId}>
                  <SelectTrigger>
                    <SelectValue placeholder="Select world" />
                  </SelectTrigger>
      ...
      {maps.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 text-lg">No maps created yet</p>
          <p className="text-gray-400">Click "Add Map" to get started</p>
        </div>
      ) : (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {maps.map((map) => (
            <Card key={map._id} className={`hover:shadow-md transition-shadow ${map.isOptimistic ? 'opacity-70 animate-pulse' : ''}`}>
                  ...
                  {!map.isOptimistic && (
                    <div className="absolute top-4 right-4 flex gap-1">
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => handleEdit(map as Doc<"maps">)}
                  ...
                  <div className="space-y-2">
                    {map.dimensions && (
                      <div className="text-xs text-gray-500">
                        <span className="font-medium">Dimensions:</span> {map.dimensions.width} × {map.dimensions.height}
                      </div>
                    )}
                    
                    {map.worldId && (
                      <div className="text-xs text-gray-500">
                        <span className="font-medium">World:</span> {getWorldName(map.worldId)}
                      </div>
                    )}
                  </div>
                </div>
